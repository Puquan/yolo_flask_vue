...
flags.DEFINE_string('media', init.EVAL.IMG_PATH,
...
def main(_argv):
    ...
    if FLAGS.media.startswith('rtsp') or FLAGS.media.isdigit() or FLAGS.media.endswith(
        ...
        for path in (os.path.join(p, name) for p, _, names in os.walk(FLAGS.media) for name in names):
            ...
            print('Inference Time:', ms, 'ms')
            image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            cv2.imwrite(init.EVAL.SAVE_PATH +'/' + path.split('/')[-1], image)
        print('Results are saved into '+init.EVAL.SAVE_PATH)
