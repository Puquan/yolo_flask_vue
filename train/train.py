...
flags.DEFINE_string('config', init.TRAIN.YAML_PATH, 'path to config file')
FLAGS = flags.FLAGS
...
def main(_argv):
    ...
    epochs = init.TRAIN.EPOCHS     #训练迭代次数


    init_weight = init.TRAIN.INIT_WEIGHT_PATH    #cfg["train"]["init_weight_path"]
    ...
    ckpt_path = os.path.join(init.TRAIN.SAVE_WEIGHT_PATH, '../back/tmp', cfg["train"]["label"],
                             time.strftime("%Y%m%d%H%M", time.localtime()))

    warmup_callback = [WarmUpScheduler(learning_rate=init.TRAIN.WARMUP_LEARN_RATE, warmup_step=init.TRAIN.WARMUP_EPOCHS * epoch_steps, verbose=1)]
